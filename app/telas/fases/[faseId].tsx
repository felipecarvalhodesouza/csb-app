    ...
    <Tela title={`Gerenciar Fase`} scroll={false}>
      <YStack f={1}>
        {loading ? (
          ...
          <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 30 }} space="$3">
            <View bg="$gray2" br="$8" p="$3" ai="center" mb="$4" shadowColor="#000" shadowOffset={{ width: 0, height: 2 }} shadowOpacity={0.10} shadowRadius={6} elevation={4}>
              <Text fontSize={16} color="$gray10" fontWeight="500" numberOfLines={1} ellipsizeMode="tail">
                Série {fase?.nome}
              </Text>
            </View>
                ...
                <YStack space="$4" mt="$6">
                  {fase.chaves.map(chave => (
                    <View key={chave.id} bg="$gray2" br="$6" p="$3" mb="$2">
                      <Text fontSize={16} fontWeight="700" color="$gray12" mb="$2" numberOfLines={1} ellipsizeMode="tail">{chave.nome}</Text>
